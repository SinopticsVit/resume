"""Generate PDF from ML_DS_Antifrod_Kurnosenko_RU.html via xhtml2pdf."""
...
def inline_css_vars(html: str) -> str:
    out = re.sub(r":root\s*\{[^}]*\}", "", html, count=1, flags=re.DOTALL)
...
def main() -> int:
    pairs = [
        (
            HERE / "ML_DS_Antifrod_Kurnosenko_RU.html",
            HERE / "ML_DS_Antifrod_Kurnosenko_RU.pdf",
        ),
    ]
    rc = 0
